...
class BloomFilter {
    ...
    public boolean contains(String s) {
        ...
        /*
        some parts of contains is similar to the add method since I followed the hint
        but this just checks bits
         */
        //like in the add method, we will go through hash functions
            for (int n = 0; n < noHashes; n++) {
                //hash code
                long hc = hashCode(s, n);
                //finding bit index with the hash code
                int bitNo = (int) (hc) & this.hashMask;
                //at the current iteration, this will check if bit is set
                if (!data.get(bitNo)) {
                    //if it is not, then element not in the set, so false is returned
                    return false;
                }
            }
            //if it is, then true
